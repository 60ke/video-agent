...
For website feature seeding, the entry path is part of the proof. Use sequential prepared screenshots and `overlay_track` callouts to show the route from homepage or product entry, to `文生图`, to the target feature, and then to the parameter page. CDP is a screenshot and coordinate-evidence producer only; dynamic click rings, highlights, and labels belong in `overlay_track`.
...
python scripts\run_pipeline_mode.py --case cases\<new_case> --mode standard --label demo_v1 --json
...
# capture/register any extra result images, then produce reviewed video_script.json
python scripts\run_pipeline_mode.py --case cases\<new_case> --mode standard --label demo_v1 --json
```
