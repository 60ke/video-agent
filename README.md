...
- 网站 UI 禁止交给 GPT Image 重画。CDP 坐标只进入离线确定性素材派生工具，不进入 `VisualPlan`、`RenderPlan`，运行时 Renderer 也不读取网页坐标。
...
首期自动补素材只生成确定性 E1：`result_detail_crop` 和 `result_vertical_layout`。手工 `materialization.json` 仍可与自动请求合并；网站入口和参数页由独立的离线确定性批处理工具生成。
