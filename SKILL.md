...
summary: Turn a product URL, approved script, CDP recipe, and generated result images into a word-timed product demo video using MiniMax TTS and Remotion.
---
...
The implementation deliberately follows the orchestration pattern of HyperFrames' `product-launch-video`, but uses this repository's stack:
...
Gate: `project.json`, `BRIEF.md`, `SCRIPT.md`, and `STYLE.md` exist; the brief states one audience, one promise, truthful proof, and one CTA.
...
Gate: the concatenated beat `voiceover` matches `SCRIPT.md`; all recipe ids and assets are registered; each beat has one clear job.
...
Gate: `check` returns `ok: true` and the final MP4 exists.
