...
CDP 只负责复用登录态、导航、等待稳定页面、截取干净截图和输出采集期语义元数据。它不录制视频、不在原图烧入红框，也不负责成片构图。
...
6. 可将 target box、可选 panel box、标签、角色和意图写入 `assets/sites/_callouts.json`，供固定派生工具构造 Prompt 和人工定位问题。

参数页精确面板裁切失败时允许保存完整页面。固定素材制作工具可以使用 callout 辅助构造派生图；V3 运行时 Renderer 不读取这些坐标，也不绘制框选。
...
坐标均为原图归一化坐标。`box` 表示语义命中目标，`panel_box` 表示构图时应保留的上下文。入口页优先提供菜单 panel；参数页优先提供整组字段 panel。它们只属于采集和固定派生阶段，不进入 `VisualPlan` 或 `RenderPlan`。
