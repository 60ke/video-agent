...
def _candidate_assets(case: CaseConfig, catalog: AssetCatalog, limit: int = 12) -> list[Asset]:
    """Return a bounded, review-approved multimodal packet in stable order."""

    preferred_roles = {"site_home", "feature_entry", "feature_form_params", "result_image", "brand_logo", "brand_ip_static"}
    candidates = [
        asset
    ...
    ordered: list[Asset] = []
    seen: set[str] = set()
    for asset in sorted(feature + shared, key=lambda item: (item.role, item.filename, item.asset_id)):
...
def plan_visual(
    ...
    assets = _candidate_assets(case, catalog)
    ...
    timing_anchors = [
        ...
        {"anchor_id": anchor.anchor_id, "frame": anchor.hit_frame, "beat_id": anchor.beat_id, "text": anchor.text}
