...
def mux_audio_tracks(plan: RenderPlan, visual_input: Path, output: Path) -> Path:
    """Apply the V3 voice/BGM/SFX mix to an already rendered visual stream."""
    ...
    if not visual_input.is_file():
        raise FileNotFoundError(f"visual render missing: {visual_input}")
    ...
    if bgm_track_index is not None:
        ...
        filters.append(f"[{bgm_label}][voice_side]sidechaincompress=threshold=0.025:ratio=8:attack=12:release=260[bgm_duck]")
    ...
    command.extend([
        "-filter_complex", ";".join(filters), "-map", "0:v:0", "-map", "[aout]", "-t", f"{duration:.6f}",
        "-c:v", "copy", "-c:a", "aac", "-b:a", "192k", "-shortest", "-movflags", "+faststart", str(output),
    ])
    proc = subprocess.run(command, capture_output=True, text=True, encoding="utf-8", errors="replace")
    if proc.returncode != 0:
        raise RuntimeError(f"ffmpeg audio mix failed: {proc.stderr[-4000:]}")
    return output


def render_video(plan: RenderPlan, output: Path, *, preset: str = "medium", crf: int = 18) -> Path:
    if plan.style.get("render_backend") == "remotion":
        from .remotion import render_remotion_video

        return render_remotion_video(plan, output, preset=preset, crf=crf)
    voice_tracks = [track for track in plan.audio_tracks if track.kind == "voice"]
    if len(voice_tracks) != 1:
        raise ValueError("V3 currently requires exactly one voice track")
