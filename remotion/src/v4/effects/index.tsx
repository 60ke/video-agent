...
const LightSweepLayer: React.FC<{effect: V4EffectProps; absoluteFrame: number}> = ({
  effect,
  absoluteFrame,
}) => {
  ...
  return (
    <AbsoluteFill style={{overflow: "hidden", backgroundColor: "#0a1018"}}>
      <AbsoluteFill
        style={{
          background: `radial-gradient(ellipse at 50% 42%, rgba(255, 244, 220, ${0.1 + 0.18 * t}) 0%, transparent 58%)`,
        }}
      />
      <div
        style={{
      ...
      <AbsoluteFill
        style={{
          opacity: 0.35 + 0.35 * Math.sin(t * Math.PI),
          boxShadow: "inset 0 0 120px rgba(255, 230, 180, 0.22)",
        }}
      />
    </AbsoluteFill>
  );
...
export const EffectStage: React.FC<Props> = ({effect, absoluteFrame, children}) => {
  if (effect.effect_id === "light_sweep") {
    return <LightSweepLayer effect={effect} absoluteFrame={absoluteFrame} />;
  }
