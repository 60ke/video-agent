...
当前阶段不建设复杂的 Prompt Registry、A/B 实验、灰度发布或可视化配置平台，只保留最小但实用的提示词外置能力。

```text
prompts/
  asset_analyst.md
  visual_opportunity.md
  materialization.md
  shot_director.md
  effect_director.md
  visual_critic.md
...
- 不修改核心业务逻辑即可调整 AI 的观察重点和审美方向；
- 不需要为提示词改动重构数据模型；
- 修改提示词后直接重跑案例；
- 后续需要更正式的版本管理时，再在此基础上升级。

当前阶段只要求在 `run_manifest.json` 中记录：

```json
{
  "prompts": {
    "asset_analyst": {
      "path": "prompts/asset_analyst.md",
      "sha256": "..."
    },
    "visual_opportunity": {
      "path": "prompts/visual_opportunity.md",
      "sha256": "..."
    },
    "visual_critic": {
      "path": "prompts/visual_critic.md",
      "sha256": "..."
    }
  }
}
...
不建设额外后台，只保证某次优秀或异常结果能够追溯到当时使用的提示词内容。

提示词可以调整：

- AI 如何理解素材；
- AI 是否优先发现前后对比、参考图到结果、局部到整体等视觉机会；
- AI 如何判断素材缺口；
- AI 选择什么镜头模式；
- AI 对构图、节奏、新鲜感和模板感的评价标准；
- 参考图造图时强调哪些保持约束。
...
- 将六类核心提示词移到 `prompts/*.md`；
- 增加统一 `prompt_loader.py`；
- 启动时读取提示词并注入当前素材、时间窗口和可用模板；
- 在 `run_manifest.json` 中记录文件路径和 SHA256；
- 暂不开发 Prompt Registry、A/B 实验、灰度和后台管理。

完成标准：

- 修改视觉提示词不需要修改核心编排代码；
- 同一运行可以追溯当时实际使用的提示词；
- 调整提示词不会改变 `timing_lock.json` 和 Cue 帧。
...
当前阶段通过少量独立的 `prompts/*.md` 文件保留视觉能力和审美能力的可调空间。后续只需修改提示词并重跑案例即可迭代 AI 编排效果，不需要立即投入复杂的提示词管理系统。
