...
| Stage 4: dependency, selection and derivation | implementation complete / Stage5 handoff revision pending | Stable resolution and repository reuse are implemented. Stage 5's first implementation unit will replace the interim precomputed capability/signature fields with a request-shape → prepare → final-signature handshake. |
| Stage 5: effect, SFX, voice and derivation registries | design revised / pending re-review | P0 review findings are incorporated: signature ownership, parameter sequence, editor members and SFX storage boundary are explicit; P1 continuity, zero-parent signature, request ID, type mapping and MotionAudioPlan draft are also defined. No Stage 5 production code has been implemented. |
...
Re-review and freeze the revised Stage 5 design before implementation. The first implementation unit is the explicit Stage4/5 DerivationRequest handshake revision, followed by typed Registry contracts and Hub validation; real GPT Image, Motion and SFX remain separate later commits. Stage 5 may consume `SpeechTimingLock` for eligibility but must emit frame-free intents; Stage 6 remains the sole owner of PhraseAnchor, subtitle cues, frame timing and SFX peak placement.
