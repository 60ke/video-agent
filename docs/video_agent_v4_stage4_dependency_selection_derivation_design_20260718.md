...
Stage 5 负责具体 Derivation Registry、Prompt 模板、模型、供应商、参数和 executor 实现。Stage 4 可先使用 fake executor 验证闭环，但在 Stage 5 完成前不得宣称真实 GPT Image 生产链路完成。
...
1. 已注册的原始 `reference_image + result_image (+ flat_plan)` 关系；
2. 已注册且 signature 匹配的派生关系；
3. 基于结果图反推参考图并注册为 E2 派生关系。
...
Resume 只在完整输入指纹一致时复用。Repository 新增素材会改变快照，从而使选择重新执行；旧 Run 的 `ResolvedAssetPlan` 仍指向历史可解析素材。
...
| s004 参数过程 | `asset_group_query(parameter_callout_sequence)` 绑定一次，按 base/stage/final 取成员 |
| s005 结果回显 | 使用 Stage 0 指定的上游 output，不重新按关键词选图 |
| s006 编辑流程 | `relation_from_input(source_result)` 精确绑定 editor_sequence |
| s007 参考→结果 | 选择或派生完整 reference_result_plan causal group |
| s008 平面图 | 从 s007 已绑定 causal group 取 flat_plan，不另找一张 |
| s009 无图承接 | `resolved_no_asset`，留给后续 LightSweep |
| s010 片尾 | `configured_asset(default_outro)` |

## 17. 实施顺序

1. 新增 Stage 4 Contracts 和 Domain Validator；
2. 实现稳定 DAG、SceneInput/Output 冻结与六类 Slot Source 分派；
3. 实现候选硬过滤、组绑定和 deterministic selector；
4. 实现 usage repository 与选择追溯；
5. 实现 MaterialGap 决策；
6. 实现 DerivationRequest、signature 复用、fake executor、注册后重查闭环；
7. 接入 V4 Orchestrator，保存完整快照与 Manifest；
8. 使用 Stage 0 fixture 验证 `s001-s010` 的素材/关系/依赖结果；
9. Stage 5 完成 Derivation Registry 后，再启用真实 GPT Image executor。
...
- [ ] 同输入快照与 seed 重跑得到完全相同的 `ResolvedAssetPlan`。
...
- [ ] Run 保存选择原因、缺口、派生请求、Repository/Registry 快照和完整输入指纹。
