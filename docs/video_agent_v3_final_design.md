...
- `generate_voice_minimax.py` 的代码默认速度仍为 `1.5`，与既定默认 `1.2` 不一致；
...
Minimax 以 `subtitle_type=word` 一次性生成整段口播和词级时间戳。默认使用 `speech-2.8-hd`、语速 `1.2`，除非 case 显式覆盖。
