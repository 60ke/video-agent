...
| P1 Registry DAG 边缺失 | Mermaid 增加 `REG -> SCENE/ASSET/BGM` |
...
```json
{
  ...
  "bgm": {"enabled": true, "profile_id": "<Unit0 必须注册并冻结的真实 BGM Profile ID>"},
...
- `bgm.enabled=true` 时必须引用 Unit 0 已注册、文件存在且可探测的真实 BGM Profile；没有真实 Profile 时不得用占位 ID 进入实现；
...
flowchart TD
    ...
    REG --> ASSET["Stage4 Asset Resolution + Derivation"]
...
Prompt 固定为 `video_agent/prompts/v4/goal_narration.md`，至少包含：
...
- [ ] 冻结 V4 Case、Production DAG、Run Manifest、CoverBrief、BgmProfile/BgmPlan 和两份 QA report Contract；
- [ ] 将 Case 示例和默认解析写死为真实 Voice Registry ID `minimax_adman_clear_01`；若改用 fixed+priority 解析，冻结唯一最高优先级和并列 fail-loud 规则；
- [ ] 冻结 `GoalNarrationResponse` JSON Schema、`goal_narration.md` Prompt、AI route key、JSON mode、repair/rebuild/escalation 和指纹字段；
- [ ] 为 BGM 注册至少一个真实可探测文件与真实 Profile ID；在此之前不得冻结 `bgm.enabled=true` 的生产默认值；
- [ ] 修订 Stage 0 §14：把“Pass B 尚未完成”更新为真实 MiniMax Pass B 已闭合，并链接 ledger/run；
- [ ] 修订 `v4_implementation_progress.md`：删除 Baseline/Stage1 的过时描述，以真实代码和黄金 conformance gate 为准；
- [ ] 冻结 seeded golden 与 production repository 两套互不替代的验收合同；
- [ ] 冻结正式发布边界：Unit 5 + Unit 6 不得拆分发布，最终同属一个 release tag；
- [ ] 冻结 V3 删除审计表，并点名 `cover._prepend_one_frame()` 不进入 V4；
