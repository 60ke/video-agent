...
| `site_callout_sequence` | 已持久化网站关键帧 | base/final 序列 | deterministic compositor | E1 |
...
| `result_to_editor_process` | 结果图 + 编辑页面底图 | editor page/modal/edited result | composite + GPT Image | E2 |
...
Stage 4 的签名命中逻辑继续生效；Stage 5 只提供真实 `execution_fingerprint`，不另造复用缓存。
...
1. 扩展 typed Registry contracts 和 Hub 交叉校验；
2. 建立五份 Registry 与冻结快照；
3. 实现 Voice fixed 解析，接回 Stage 1 TTS 前；
4. 实现 Derivation capability resolver、PreparedDerivation 和生产禁用 Fake；
5. 按 capability 拆分 Prompt 与 GPT Image/确定性 executor；
6. 接入 Stage 3 单事务注册和 Stage 4 重查；
7. 将动效硬编码迁到 Effect Registry，实现 Motion Assignment；
8. 将 SFX Catalog/Profile 迁到 Registry，输出无帧号 SFX Intent；
9. 冻结 `MotionAudioPlan`，为 Stage 6 提供稳定输入；
10. 更新进度文档，但不切换 V3 主线。
